            ...
            data_version: &String, pool : &Pool<Postgres>) -> Result<(), AppError>
{
    ...
    write_name_info(output_file_str, &vcode, pool, &singvals).await?;
...
async fn write_explanation(output_file_str: &str) -> Result<(), AppError> {
    
    append_to_file(output_file_str, &get_hdr_line("ABBREVIATIONS AND TOTALS"))?;
    ...
    append_to_file(output_file_str, &expl_text)?;
...
async fn write_name_info(output_file_str: &str, vcode: &String, pool: &Pool<Postgres>, 
    ...
    append_to_file(output_file_str, &dup_text)?;
...
async fn write_name_wolc_info(output_file_str: &str, vcode: &String, pool: &Pool<Postgres>
    ...
    append_to_file(output_file_str, &wolc_text)?;
...
async fn write_ranked_name_info(output_file_str: &str, vcode: &String, pool: &Pool<Postgres>, 
    ...
    let s_text =get_sing_hdr() + &get_singleton_line(&s1.description, s1.number, s1.pc)
                   + &get_singleton_line(&s2.description, s2.number, s2.pc)
                   + &get_singleton_line(&s3.description, s3.number, s3.pc);
    append_to_file(output_file_str, &s_text)?;
...
async fn write_ror_name_details(output_file_str: &str, singvals: &HashMap<String, Singleton>) -> Result<(), AppError> {
