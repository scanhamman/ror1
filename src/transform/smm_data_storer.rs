...
use super::smm_helper::FileParams;
...
pub async fn store_summary_data (pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let dv_dt = "select \'".to_string() + &vcode + "\' as vcode, \'" 
                     +  &fp.vdate_as_string + "\'::date as vdate, ";
    ...
    smm_helper::delete_any_existing_data(&vcode, &fp.vdate_as_string, pool).await?;
    ...
    sqlx::query(sql).bind(&vcode).bind(vdate)
    ...
    smm_helper::create_name_attributes(&dv_dt, &num_orgs_str, &num_names.to_string(), pool).await?;

    smm_helper::create_other_attributes(&dv_dt, &num_orgs_str, &num_types.to_string(), &num_ext_ids.to_string(), 
                            &num_links.to_string(), &num_rels.to_string(), pool).await?;

    smm_helper::create_count_distributions(&dv_dt, &num_orgs_str, pool).await?;     
    ...
    smm_helper::create_ranked_count_distributions(&vcode, vdate, &dv_dt, num_names, num_locations,  
                                    num_ne, num_nltn, num_nus, pool).await?;   

    smm_helper::create_type_linked_tables(&dv_dt, pool).await?;

    smm_helper::store_singletons(&vcode, vdate, num_orgs, num_names, num_nltn, pool).await?;
